...
---
Use drupal/redirect to create and manage HTTP redirects. Programmatic redirects are Redirect entities with redirect_source, redirect_redirect, and status_code; source paths should omit the leading slash, while destinations can use forms such as internal:/path or entity:node/1. Query redirects through the redirect entity storage, for example loadByProperties(['redirect_source__path' => 'old-path']). To customize redirect handling, subscribe to RedirectEvents::REDIRECT and inspect or modify the RedirectEvent. Enable the redirect_404 submodule when 404 handling should feed redirect creation or management.
